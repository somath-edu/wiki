...
## 6. PDF 문제지 자동 배포 (Typst)
프로젝트는 GitHub Actions를 통해 `.typ` 소스를 자동으로 PDF로 변환하여 배포합니다.
- **소스 경로**: `print/[단원]/[파일명].typ`
- **배포 경로**: `web/public/pdf/[단원]/[파일명].pdf`
- **추가 방법**: `.github/workflows/deploy.yml`의 `Compile Typst to PDF` 단계를 수정합니다.

## 7. 사서의 사명
"수집된 원본 정보를 체계적인 수학교과서 데이터로 정제하고, 이를 아름답고 고급스러운 웹 페이지와 고품질 PDF 문제지로 시각화한다."
