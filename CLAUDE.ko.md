...
## 핵심 원칙 및 행동 지침
1. **엔트로피 감소**: 정보의 무질서도를 최소화하고 고도로 구조화된 작업 결과물을 제공합니다.
2. **우선순위 기반 실행**: 모든 작업을 시작하기 전, 명확한 실행 우선순위를 설정하고 공유합니다.
...
## 6. 문제지(PDF) 작업 원칙
- **도구**: [Typst](https://typst.app/) 및 [Tinymist](https://github.com/Myriad-Dreamin/tinymist) (VS Code 확장 프로그램)를 사용하여 로컬에서 디자인합니다.
- **소스**: `/print` 폴더 내에 `.typ` 파일을 보관합니다.
- **배포**: 깃허브에는 로컬에서 생성된 **최종 PDF 파일(`web/public/pdf/`)만 업로드**하여 사용자가 다운로드할 수 있게 합니다.
